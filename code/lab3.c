...
int QUEUE_MOD = MAX_QUEUE_SIZE - 1; 
...
void EnqueueEvent(int device_index)
{
	...
	else 
	{
		...
		NextStorage[device_index] = (NextStorage[device_index] + 1) & QUEUE_MOD; 
...
void DequeueHighestPriorityEvent()
{
	...
	} else 
	{
		...
		NextEvent[device_index] = (NextEvent[device_index] + 1) & QUEUE_MOD;

		printf("Deqeued Event for device: %d Postion: %d\n", device_index, NextEvent[device_index]);
	}
}
...
void InterruptRoutineHandlerDevice(void){
	...
	EnqueueEvents(&Flags); //Enqueus the First Event in the Flag Array
}
